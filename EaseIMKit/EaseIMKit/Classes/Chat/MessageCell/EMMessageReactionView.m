...
- (void)layoutSubviews {
    ...
    CGFloat w = 0;
    if (_reactionList.count == 1 && _reactionList.firstObject.count == 1) {
        w = 28;
        _countLabel.hidden = YES;
        _moreImageView.hidden = YES;
    } else {
        if (_reactionList.count > 5) {
            w = 124 + _countLabelWidth;
            _moreImageView.frame = CGRectMake(90, 0, 11, self.bounds.size.height);
            _moreImageView.hidden = NO;
        } else {
            w = 20 + 20 * _reactionList.count + _countLabelWidth;
            _moreImageView.hidden = YES;
        }
...
- (void)setReactionList:(NSArray<EMMessageReaction *> *)reactionList {
    ...
    if (reactionList.count > _imageViews.count) {
        NSUInteger loopCount = reactionList.count - _imageViews.count;
    ...
    for (int i = 0; i < _imageViews.count; i ++) {
        if (i < reactionList.count) {
            _imageViews[i].hidden = NO;
            _imageViews[i].image = [UIImage easeUIImageNamed:reactionList[i].reaction];
            if (_reactionList.count == 1) {
                _imageViews[i].frame = CGRectMake(6, 6, 16, 16);
            } else {
                _imageViews[i].frame = CGRectMake(10 + 20 * i, 0, 16, 16);
            }
        } else {
