...
@implementation EaseRemindManager
+ (void)remindMessage:(EMMessage *)aMessage {
    [[EaseRemindManager shared] remindMessage:aMessage];
}

+ (void)updateApplicationIconBadgeNumber:(NSInteger)aBadgeNumber {
    [[EaseRemindManager shared] updateApplicationIconBadgeNumber:aBadgeNumber];
}

// 播放等待铃声
+ (void)playWattingSound {
    [[EaseRemindManager shared] _playWattingSound];
}

// 播放铃声
+ (void)playRing:(BOOL)playVibration{
    [[EaseRemindManager shared] _playRing:playVibration];
}

// 停止铃声
+ (void)stopSound {
    [[EaseRemindManager shared] _stopSound];
}

// 振动
+ (void)playVibration {
    [[EaseRemindManager shared] _playVibration:NULL];
}

#pragma - mark private
+ (EaseRemindManager *)shared {
    static EaseRemindManager *remindManager_;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        remindManager_ = [[EaseRemindManager alloc] init];
    });
...
{
    if ([EaseRemindManager shared].player) {
