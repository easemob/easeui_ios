...
static NSArray *EaseBundlePreferredScales() {
...
@implementation EaseImageAssetManager {
    Ease_LOCK_DECLARE(_lock);
}

+ (instancetype)sharedAssetManager {
    static dispatch_once_t onceToken;
    static EaseImageAssetManager *assetManager;
    dispatch_once(&onceToken, ^{
        assetManager = [[EaseImageAssetManager alloc] init];
    });
    return assetManager;
}
...
- (NSString *)getPathForName:(NSString *)name bundle:(NSBundle *)bundle preferredScale:(CGFloat *)scale {
    ...
    NSArray *scales = EaseBundlePreferredScales();
