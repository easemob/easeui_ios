 ...
 */
@interface UIImage (EaseMetadata)
 ...
 */
@property (nonatomic, assign) NSUInteger ease_imageLoopCount;
 ...
 */
@property (nonatomic, assign, readonly) BOOL ease_isAnimated;
 ...
 */
@property (nonatomic, assign, readonly) BOOL ease_isVector;

/**
 * The image format represent the original compressed image data format.
 * If you don't manually specify a format, this information is retrieve from CGImage using `CGImageGetUTType`, which may return nil for non-CG based image. At this time it will return `EaseImageFormatUndefined` as default value.
 ...
 */
@property (nonatomic, assign) EaseImageFormat ease_imageFormat;

/**
 A bool value indicating whether the image is during incremental decoding and may not contains full pixels.
 */
@property (nonatomic, assign) BOOL ease_isIncremental;
