 ...
 */
@interface EaseImageCacheConfig : NSObject <NSCopying>

/**
 Gets the default cache config used for shared instance or initialization when it does not provide any cache config. Such as `EaseImageCache.sharedImageCache`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) EaseImageCacheConfig *defaultCacheConfig;
...
/*
 * The option to control weak memory cache for images. When enable, `EaseImageCache`'s memory cache will use a weak maptable to store the image at the same time when it stored to memory, and get removed at the same time.
 ...
 */
@property (assign, nonatomic) EaseImageCacheConfigExpireType diskCacheExpireType;
...
/**
 * The custom memory cache class. Provided class instance must conform to `EaseMemoryCache` protocol to allow usage.
 * Defaults to built-in `EaseMemoryCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initialized has no effect.
 */
@property (assign, nonatomic, nonnull) Class memoryCacheClass;

/**
 * The custom disk cache class. Provided class instance must conform to `EaseDiskCache` protocol to allow usage.
 * Defaults to built-in `EaseDiskCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initialized has no effect.
 */
