...
/**
 This is the protocol for EaseAnimatedImage class only but not for EaseAnimatedImageCoder. If you want to provide a custom animated image class with full advanced function, you can conform to this instead of the base protocol.
 */
@protocol EaseAnimatedImage <EaseAnimatedImageProvider>
 ...
 */
- (nullable instancetype)initWithData:(nonnull NSData *)data scale:(CGFloat)scale options:(nullable EaseImageCoderOptions *)options;

/**
 Initializes the image with an animated coder. You can use the coder to decode the image frame later.
 @note We use this with animated coder which conforms to `EaseProgressiveImageCoder` for progressive animation decoding.
 
 @param animatedCoder An animated coder which conform `EaseAnimatedImageCoder` protocol
 @param scale The scale factor to assume when interpreting the image data. Applying a scale factor of 1.0 results in an image whose size matches the pixel-based dimensions of the image. Applying a different scale factor changes the size of the image as reported by the `size` property.
 @return An initialized object
 */
- (nullable instancetype)initWithAnimatedCoder:(nonnull id<EaseAnimatedImageCoder>)animatedCoder scale:(CGFloat)scale;
...
/**
 ...
 @note We use this with animated coder which conforms to `EaseProgressiveImageCoder` for progressive animation decoding.
 */
@property (nonatomic, strong, readonly, nullable) id<EaseAnimatedImageCoder> animatedCoder;

@end

/**
 The image class which supports animating on `EaseAnimatedImageView`. You can also use it on normal UIImageView/NSImageView.
 */
@interface EaseAnimatedImage : UIImage <EaseAnimatedImage>
...
// Pay attention, when the animated image frame count <= 1, all the `EaseAnimatedImageProvider` protocol methods will return nil or 0 value, you'd better check the frame count before usage and keep fallback.
 ...
 */
@property (nonatomic, assign, readonly) EaseImageFormat animatedImageFormat;
...
// By default, animated image frames are returned by decoding just in time without keeping into memory. But you can choose to preload them into memory as well, See the description in `EaseAnimatedImage` protocol.
