...
@interface EaseAnimatedImagePlayer : NSObject
...
/// The provider can be any protocol implementation, like `EaseAnimatedImage`, `EaseImageGIFCoder`, etc.
...
- (nullable instancetype)initWithProvider:(nonnull id<EaseAnimatedImageProvider>)provider;

/// Create a player with animated image provider. If the provider's `animatedImageFrameCount` is less than 1, returns nil.
/// The provider can be any protocol implementation, like `EaseAnimatedImage` or `EaseImageGIFCoder`, etc.
/// @note This provider can represent mutable content, like progressive animated loading. But you need to update the frame count by yourself
/// @param provider The animated provider
+ (nullable instancetype)playerWithProvider:(nonnull id<EaseAnimatedImageProvider>)provider;
