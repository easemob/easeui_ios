...
/**
 ...
 For others (macOS/watchOS or iOS/tvOS 10-), these method use the `EaseImageGraphics.h` to implements the same behavior (but without dynamic bitmap support)
*/

typedef void (^EaseGraphicsImageDrawingActions)(CGContextRef _Nonnull context);
typedef NS_ENUM(NSInteger, EaseGraphicsImageRendererFormatRange) {
    EaseGraphicsImageRendererFormatRangeUnspecified = -1,
    EaseGraphicsImageRendererFormatRangeAutomatic = 0,
    EaseGraphicsImageRendererFormatRangeExtended,
    EaseGraphicsImageRendererFormatRangeStandard
};

/// A set of drawing attributes that represent the configuration of an image renderer context.
@interface EaseGraphicsImageRendererFormat : NSObject
...
@property (nonatomic) EaseGraphicsImageRendererFormatRange preferredRange;
...
@interface EaseGraphicsImageRenderer : NSObject
...
/// @param size The size of images output from the renderer, specified in points.
/// @param format A EaseGraphicsImageRendererFormat object that encapsulates the format used to create the renderer context.
/// @return An initialized image renderer.
- (nonnull instancetype)initWithSize:(CGSize)size format:(nonnull EaseGraphicsImageRendererFormat *)format;

/// Creates an image by following a set of drawing instructions.
/// @param actions A EaseGraphicsImageDrawingActions block that, when invoked by the renderer, executes a set of drawing instructions to create the output image.
...
- (nonnull UIImage *)imageWithActions:(nonnull NS_NOESCAPE EaseGraphicsImageDrawingActions)actions;
