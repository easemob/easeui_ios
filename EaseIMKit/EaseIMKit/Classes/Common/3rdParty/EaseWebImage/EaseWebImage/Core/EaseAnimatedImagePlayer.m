...
#import "EaseDeviceHelper.h"
#import "EaseInternalMacros.h"

@interface EaseAnimatedImagePlayer () {
    Ease_LOCK_DECLARE(_lock);
...
@property (nonatomic, strong) id<EaseAnimatedImageProvider> animatedProvider;
...
@property (nonatomic, strong) EaseDisplayLink *displayLink;

@end

@implementation EaseAnimatedImagePlayer

- (instancetype)initWithProvider:(id<EaseAnimatedImageProvider>)provider {
    self = [super init];
    if (self) {
...
+ (instancetype)playerWithProvider:(id<EaseAnimatedImageProvider>)provider {
    EaseAnimatedImagePlayer *player = [[EaseAnimatedImagePlayer alloc] initWithProvider:provider];
    return player;
}
...
- (EaseDisplayLink *)displayLink {
    if (!_displayLink) {
        _displayLink = [EaseDisplayLink displayLinkWithTarget:self selector:@selector(displayDidRefresh:)];
...
- (void)setupCurrentFrame {
    ...
    if (self.playbackMode == EaseAnimatedImagePlaybackModeReverse ||
               self.playbackMode == EaseAnimatedImagePlaybackModeReversedBounce) {
...
- (void)stopPlaying {
    [_fetchQueue cancelAllOperations];
    // Using `_displayLink` here because when UIImageView dealloc, it may trigger `[self stopAnimating]`, we already release the display link in EaseAnimatedImageView's dealloc method.
    [_displayLink stop];
...
- (void)displayDidRefresh:(EaseDisplayLink *)displayLink {
    ...
    } else if (self.playbackMode == EaseAnimatedImagePlaybackModeBounce ||
               self.playbackMode == EaseAnimatedImagePlaybackModeReversedBounce) {
    ...
    if (!fetchFrame && !bufferFull && self.fetchQueue.operationCount == 0) {
        ...
        id<EaseAnimatedImageProvider> animatedProvider = self.animatedProvider;
        @weakify(self);
...
- (void)calculateMaxBufferCount {
    ...
    if (self.maxBufferSize > 0) {
        ...
        NSUInteger total = [EaseDeviceHelper totalMemory];
        NSUInteger free = [EaseDeviceHelper freeMemory];
