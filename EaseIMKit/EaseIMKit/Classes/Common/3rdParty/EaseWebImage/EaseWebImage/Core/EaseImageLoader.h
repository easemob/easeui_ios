...
typedef void(^EaseImageLoaderProgressBlock)(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL);
typedef void(^EaseImageLoaderCompletedBlock)(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished);

#pragma mark - Context Options

/**
 A `UIImage` instance from `EaseWebImageManager` when you specify `EaseWebImageRefreshCached` and image cache hit.
 This can be a hint for image loader to load the image from network and refresh the image from remote location if needed. If the image from remote location does not change, you should call the completion with `EaseWebImageErrorCacheNotModified` error. (UIImage)
 @note If you don't implement `EaseWebImageRefreshCached` support, you do not need to care about this context option.
 */
FOUNDATION_EXPORT EaseWebImageContextOption _Nonnull const EaseWebImageContextLoaderCachedImage;

#pragma mark - Helper method

/**
 This is the built-in decoding process for image download from network or local file.
 @note If you want to implement your custom loader with `requestImageWithURL:options:context:progress:completed:` API, but also want to keep compatible with EaseWebImage's behavior, you'd better use this to produce image.
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable EaseImageLoaderDecodeImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, EaseWebImageOptions options, EaseWebImageContext * _Nullable context);

/**
 This is the built-in decoding process for image progressive download from network. It's used when `EaseWebImageProgressiveLoad` option is set. (It's not required when your loader does not support progressive image loading)
 @note If you want to implement your custom loader with `requestImageWithURL:options:context:progress:completed:` API, but also want to keep compatible with EaseWebImage's behavior, you'd better use this to produce image.
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable EaseImageLoaderDecodeProgressiveImageData(NSData * _Nonnull imageData, NSURL * _Nonnull imageURL, BOOL finished,  id<EaseWebImageOperation> _Nonnull operation, EaseWebImageOptions options, EaseWebImageContext * _Nullable context);
...
/**
 ...
 If you want to implement custom loader for image download from network or local file, you just need to concentrate on image data download only. After the download finish, call `EaseImageLoaderDecodeImageData` or `EaseImageLoaderDecodeProgressiveImageData` to use the built-in decoding process and produce image (Remember to call in the global queue). And finally callback the completion block.
 If you directly get the image instance using some third-party EaseKs, such as image directly from Photos framework. You can process the image data and image instance by yourself without that built-in decoding process. And finally callback the completion block.
 @note It's your responsibility to load the image in the desired global queue(to avoid block main queue). We do not dispatch these method call in a global queue but just from the call queue (For `EaseWebImageManager`, it typically call from the main queue).
*/
@protocol EaseImageLoader <NSObject>
 ...
 */
- (nullable id<EaseWebImageOperation>)requestImageWithURL:(nullable NSURL *)url
                                                options:(EaseWebImageOptions)options
                                                context:(nullable EaseWebImageContext *)context
                                               progress:(nullable EaseImageLoaderProgressBlock)progressBlock
                                              completed:(nullable EaseImageLoaderCompletedBlock)completedBlock;


/**
 Whether the error from image loader should be marked indeed un-recoverable or not.
 If this return YES, failed URL which does not using `EaseWebImageRetryFailed` will be blocked into black list. Else not.
