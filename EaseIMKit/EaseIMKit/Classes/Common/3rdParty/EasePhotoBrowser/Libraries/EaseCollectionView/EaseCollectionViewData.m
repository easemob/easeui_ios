...
@interface EaseCollectionViewData () {
...
@property (nonatomic, unsafe_unretained) EaseCollectionViewLayout *layout;
@property (nonatomic, strong) NSArray *cachedLayoutAttributes;

@end

@implementation EaseCollectionViewData
...
- (void)validateLayoutInRect:(CGRect)rect {
    ...
    if (!CGRectEqualToRect(_validLayoutRect, rect)) {
        ...
        self.cachedLayoutAttributes = [[self.layout layoutAttributesForElementsInRect:rect] filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(EaseCollectionViewLayoutAttributes *evaluatedObject, NSDictionary *bindings) {
            return ([evaluatedObject isKindOfClass:EaseCollectionViewLayoutAttributes.class] &&
