//
//  EaseCollectionViewFlowLayout.m
//  PSPDFKit
//
...
@implementation EaseCollectionViewFlowLayout {
    ...
    EaseCollectionViewScrollDirection _scrollDirection;
...
- (void)commonInit {
    ...
    _scrollDirection = EaseCollectionViewScrollDirectionVertical;
...
///////////////////////////////////////////////////////////////////////////////////////////
#pragma mark - EaseCollectionViewLayout
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    for (EaseGridLayoutSection *section in _data.sections) {
        if (CGRectIntersectsRect(section.frame, rect)) {
            ...
            if (!CGRectIsEmpty(normalizedHeaderFrame) && CGRectIntersectsRect(normalizedHeaderFrame, rect)) {
                EaseCollectionViewLayoutAttributes *layoutAttributes = [[self.class layoutAttributesClass] layoutAttributesForSupplementaryViewOfKind:PSTCollectionElementKindSectionHeader withIndexPath:[NSIndexPath indexPathForItem:0 inSection:sectionIndex]];
            ...
            for (EaseGridLayoutRow *row in section.rows) {
                ...
                if (CGRectIntersectsRect(normalizedRowFrame, rect)) {
                    ...
                    for (NSInteger itemIndex = 0; itemIndex < row.itemCount; itemIndex++) {
                        EaseCollectionViewLayoutAttributes *layoutAttributes;
                        ...
                        if (row.fixedItemSize) {
                            ...
                            EaseGridLayoutItem *item = row.items[itemIndex];
            ...
            if (!CGRectIsEmpty(normalizedFooterFrame) && CGRectIntersectsRect(normalizedFooterFrame, rect)) {
                EaseCollectionViewLayoutAttributes *layoutAttributes = [[self.class layoutAttributesClass] layoutAttributesForSupplementaryViewOfKind:PSTCollectionElementKindSectionFooter withIndexPath:[NSIndexPath indexPathForItem:0 inSection:sectionIndex]];
...
- (EaseCollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    EaseGridLayoutSection *section = _data.sections[indexPath.section];
    EaseGridLayoutRow *row = nil;
    ...
    if (section.fixedItemSize && indexPath.item / section.itemsByRowCount < (NSInteger)section.rows.count) {
        ...
        EaseGridLayoutItem *item = section.items[indexPath.item];
        row = item.rowObject;
        itemFrame = item.itemFrame;
    }

    EaseCollectionViewLayoutAttributes *layoutAttributes = [[self.class layoutAttributesClass] layoutAttributesForCellWithIndexPath:indexPath];
...
- (EaseCollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    EaseCollectionViewLayoutAttributes *layoutAttributes = nil;

    if (sectionIndex < _data.sections.count) {
        EaseGridLayoutSection *section = _data.sections[sectionIndex];
...
- (EaseCollectionViewLayoutAttributes *)layoutAttributesForDecorationViewWithReuseIdentifier:(NSString *)identifier atIndexPath:(NSIndexPath *)indexPath {
    return nil;
}
...
- (BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
    ...
    if ((_visibleBounds.size.width != newBounds.size.width && self.scrollDirection == EaseCollectionViewScrollDirectionVertical) || (_visibleBounds.size.height != newBounds.size.height && self.scrollDirection == EaseCollectionViewScrollDirectionHorizontal)) {
...
- (void)prepareLayout {
    ...
    _data = [EaseGridLayoutInfo new]; // clear old layout data
    _data.horizontal = self.scrollDirection == EaseCollectionViewScrollDirectionHorizontal;
    _visibleBounds = self.collectionView.bounds;
...
//    NSAssert(_data.sections.count == 0, @"Grid layout is already populated?");
    ...
    id<EaseCollectionViewDelegateFlowLayout> flowDataSource = (id<EaseCollectionViewDelegateFlowLayout>)self.collectionView.delegate;
    ...
    for (NSUInteger section = 0; section < numberOfSections; section++) {
        EaseGridLayoutSection *layoutSection = [_data addSection];
        ...
        if (implementsSizeDelegate) {
            for (NSUInteger item = 0; item < numberOfItems; item++) {
                ...
                EaseGridLayoutItem *layoutItem = [layoutSection addItem];
