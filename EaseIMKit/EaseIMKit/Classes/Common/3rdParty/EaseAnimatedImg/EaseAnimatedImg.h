...
extern const NSTimeInterval kEaseAnimatedImgDelayTimeIntervalMinimum;

//
//  An `EaseAnimatedImg`'s job is to deliver frames in a highly performant way and works in conjunction with `EaseAnimatedImgView`.
...
//
@interface EaseAnimatedImg : NSObject
...
// Pass either a `UIImage` or an `EaseAnimatedImg` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `EaseAnimatedImg` with all fields initialized, on failure they return `nil` and an error will be logged.
