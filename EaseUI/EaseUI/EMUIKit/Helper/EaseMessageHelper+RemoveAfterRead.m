...
- (void)didReceiveHasReadAcks:(NSArray *)aMessages
{
    dispatch_async(_queue, ^{
        NSMutableArray *removeMessages = nil;
        for (EMMessage *message in aMessages)
        {
            if ([self handleReceiveHasReadResponse:message])
            {
                if (!removeMessages) {
                    removeMessages = [NSMutableArray array];
                }
        ...
        if (removeMessages)
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                [multicastDelegate emHelper:self handleRemoveAfterReadMessage:removeMessages];
            });
        }
    });
}
...
- (void)sendRemoveMessageAction:(EMMessage *)aMessage
{
    ...
    dispatch_async(_queue, ^{
        if ([weakSelf isConnected]) {
            [weakSelf.chatManager asyncSendReadAckForMessage:aMessage];
    ...
    dispatch_async(_queue, ^{
        if ([weakSelf isConnected]) {
            for (NSString *chatter in [weakSelf.needRemoveDic allKeys]) {
                EMConversation *conversation = [weakSelf.chatManager getConversation:chatter
                ...
                for (NSString *messageId in weakSelf.needRemoveDic[chatter]) {
                    ...
                    [weakSelf.chatManager asyncSendReadAckForMessage:message];
                    [conversation deleteMessageWithId:messageId];
                }
                if (!conversation.latestMessage) {
                    [weakSelf.chatManager deleteConversation:conversation.conversationId deleteMessages:NO];
                }
            }
 ...
 *
 */
- (BOOL)handleReceiveHasReadResponse:(EMMessage *)message
{
    if (![EaseMessageHelper isRemoveAfterReadMessage:message])
    {
        return NO;
    }
    EMConversation *conversation = [self.chatManager getConversation:message.conversationId type:EMConversationTypeChat createIfNotExist:NO];
    if (!conversation)
    {
        return NO;
    }
    if ([conversation deleteMessageWithId:message.messageId])
    {
        if (!conversation.latestMessage) {
...
- (void)handleRemoveAfterReadMessage:(EMMessage *)message;
{
    ...
    dispatch_async(_queue, ^{
        EMConversation *conversation = [self.chatManager getConversation:message.conversationId type:(EMConversationType)message.chatType createIfNotExist:NO];
        if (!conversation) {
            return;
        }
        if (![message.from isEqualToString:self.account])
        {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [multicastDelegate emHelper:self handleRemoveAfterReadMessage:@[message]];
            });
        }
    });
}
