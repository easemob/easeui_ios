...
+ (BOOL)isInputCMDMessage:(EMMessage *)message
{
    ...
    if (![message.messageBodies.firstObject isKindOfClass:[EMCommandMessageBody class]])
    {
        return NO;
    }
    EMCommandMessageBody *body = (EMCommandMessageBody *)[message.messageBodies firstObject];
    if (!body)
    {
        return NO;
    }
...
{
    if (conversationType == eConversationTypeChat &&
...
+ (void)canSendInputCmdMsgToChatter:(NSString *)chatter status:(BOOL)isInput
{
    //输入提示未开启，或 未连接，不发送
    if (![EaseMessageHelper inputStateIsValid] ||
        ![[EaseMessageHelper sharedInstance].chatManager isConnected])
    {
        return;
    }
    BOOL isCanSend = NO;
...
+ (void)sendInputStateCmdMessage:(NSString *)aChatter status:(BOOL)isInput
{
    ...
    cmdChat.cmd = KEM_INPUTSTATE_CMDACTION;
    EMCommandMessageBody *body = [[EMCommandMessageBody alloc] initWithChatObject:cmdChat];
    // 生成message
    EMMessage *message = [[EMMessage alloc] initWithReceiver:aChatter bodies:@[body]];
    message.messageType = eMessageTypeChat;
    message.requireEncryption = NO;
    message.ext = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:isInput], KEM_INPUTSTATE_STATE, nil];
