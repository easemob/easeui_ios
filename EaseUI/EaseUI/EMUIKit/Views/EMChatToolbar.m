...
@interface EMChatToolbar()<UITextViewDelegate, EMFaceDelegate>
...
{
    if (_recordView == nil) {
        _recordView = [[EMRecordView alloc] initWithFrame:CGRectMake(90, 130, 140, 140)];
    }
...
{
    if (_faceView == nil) {
        _faceView = [[EMFaceView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_toolbarView.frame), self.frame.size.width, 180)];
        [(EMFaceView *)_faceView setDelegate:self];
...
{
    if (_moreView == nil) {
        _moreView = [[EMChatBarMoreView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_toolbarView.frame), self.frame.size.width, 80) type:self.chatBarType];
...
{
    _delegate = delegate;
    if ([_moreView isKindOfClass:[EMChatBarMoreView class]]) {
        [(EMChatBarMoreView *)_moreView setDelegate:delegate];
    }
}
...
- (void)selectedFacialView:(NSString *)str isDelete:(BOOL)isDelete
{
    ...
    if (!isDelete && str.length > 0) {
        ...
        if (chatText.length >= 2)
        {
            ...
            if ([(EMFaceView *)self.faceView stringIsFace:subStr]) {
