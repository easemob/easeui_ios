...
@implementation EaseImageCachesManagerOperation {
    Ease_LOCK_DECLARE(_pendingCountLock);
}
...
- (instancetype)init {
    if (self = [super init]) {
        Ease_LOCK_INIT(_pendingCountLock);
        _pendingCount = 0;
    }
    return self;
}
...
- (NSUInteger)pendingCount {
    Ease_LOCK(_pendingCountLock);
    NSUInteger pendingCount = _pendingCount;
    Ease_UNLOCK(_pendingCountLock);
    return pendingCount;
}
