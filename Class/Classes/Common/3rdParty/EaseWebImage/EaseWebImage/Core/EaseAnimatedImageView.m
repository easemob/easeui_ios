...
@interface EaseAnimatedImageView () {
    BOOL _initFinished; // Extra flag to mark the `commonInit` is called
    NSRunLoopMode _runLoopMode;
    NSUInteger _maxBufferSize;
    double _playbackRate;
    EaseAnimatedImagePlaybackMode _playbackMode;
}

@property (nonatomic, strong, readwrite) EaseAnimatedImagePlayer *player;
...
@implementation EaseAnimatedImageView
#if Ease_UIKIT
...
+ (instancetype)imageViewWithImage:(NSImage *)image
{
    ...
    EaseAnimatedImageView *imageView = [[EaseAnimatedImageView alloc] initWithFrame:frame];
...
- (void)setImage:(UIImage *)image
{
    ...
    if ([image.class conformsToProtocol:@protocol(EaseAnimatedImage)]) {
        if (!self.player) {
            id<EaseAnimatedImageProvider> provider;
            // Check progressive loading
            if (self.isProgressive) {
                provider = [self progressiveAnimatedCoderForImage:image];
            } else {
                provider = (id<EaseAnimatedImage>)image;
            }
            // Create animated player
            self.player = [EaseAnimatedImagePlayer playerWithProvider:provider];
        } else {
            // Update Frame Count
            self.player.totalFrameCount = [(id<EaseAnimatedImage>)image animatedImageFrameCount];
        }
...
- (void)setPlaybackMode:(EaseAnimatedImagePlaybackMode)playbackMode {
    _playbackMode = playbackMode;
    self.player.playbackMode = playbackMode;
}

- (EaseAnimatedImagePlaybackMode)playbackMode {
    if (!_initFinished) {
        return EaseAnimatedImagePlaybackModeNormal; // Default mode is normal
    }
...
- (void)checkPlay
{
    // Only handle for EaseAnimatedImage, leave UIAnimatedImage or animationImages for super implementation control
...
- (void)updateIsProgressiveWithImage:(UIImage *)image
{
    ...
    id<EaseAnimatedImageCoder> currentAnimatedCoder = [self progressiveAnimatedCoderForImage:image];
    if (currentAnimatedCoder) {
        ...
        if (!previousImage) {
            ...
            id<EaseAnimatedImageCoder> previousAnimatedCoder = [self progressiveAnimatedCoderForImage:previousImage];
...
- (id<EaseAnimatedImageCoder, EaseProgressiveImageCoder>)progressiveAnimatedCoderForImage:(UIImage *)image
{
    if ([image.class conformsToProtocol:@protocol(EaseAnimatedImage)] && image.ease_isIncremental && [image respondsToSelector:@selector(animatedCoder)]) {
        id<EaseAnimatedImageCoder> animatedCoder = [(id<EaseAnimatedImage>)image animatedCoder];
        if ([animatedCoder conformsToProtocol:@protocol(EaseProgressiveImageCoder)]) {
            return (id<EaseAnimatedImageCoder, EaseProgressiveImageCoder>)animatedCoder;
        }
    }
    return nil;
}
...
- (NSView *)imageView {
    NSImageView *imageView = objc_getAssociatedObject(self, Ease_SEL_SPI(imageView));
    if (!imageView) {
        // macOS 10.14
        imageView = objc_getAssociatedObject(self, Ease_SEL_SPI(imageSubview));
    }
    return imageView;
}
