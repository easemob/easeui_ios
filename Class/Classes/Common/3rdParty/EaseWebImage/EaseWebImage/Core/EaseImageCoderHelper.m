...
static inline size_t EaseByteAlign(size_t size, size_t alignment) {
...
/*
 * Defines the maximum size in MB of the decoded image when the flag `EaseWebImageScaleDownLargeImages` is set
...
@implementation EaseImageCoderHelper

+ (UIImage *)animatedImageWithFrames:(NSArray<EaseImageFrame *> *)frames {
    ...
    [frames enumerateObjectsUsingBlock:^(EaseImageFrame * _Nonnull frame, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    CFStringRef imageUTType = [NSData Ease_UTTypeFromImageFormat:EaseImageFormatGIF];
    ...
    EaseAnimatedImageRep *imageRep = [[EaseAnimatedImageRep alloc] initWithData:imageData];
...
+ (NSArray<EaseImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
    if (!animatedImage) {
        return nil;
    }
    ...
    [animatedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull image, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([image isEqual:previousImage]) {
            ...
            EaseImageFrame *frame = [EaseImageFrame frameWithImage:previousImage duration:avgDuration * repeatCount];
    ...
    for (size_t i = 0; i < frameCount; i++) {
        @autoreleasepool {
            ...
            EaseImageFrame *frame = [EaseImageFrame frameWithImage:frameImage duration:frameDuration];
            [frames addObject:frame];
        }
    }
#endif
...
+ (CGImageRef)CGImageCreateDecoded:(CGImageRef)cgImage orientation:(CGImagePropertyOrientation)orientation {
    ...
    // Apply transform
    CGAffineTransform transform = EaseCGContextTransformFromOrientation(orientation, CGSizeMake(newWidth, newHeight));
...
+ (CGImageRef)CGImageCreateScaled:(CGImageRef)cgImage size:(CGSize)size {
    ...
    output_buffer.rowBytes = EaseByteAlign(output_buffer.width * 4, 64);
...
+ (UIImage *)decodedImageWithImage:(UIImage *)image {
    ...
    EaseImageCopyAssociatedObject(image, decodedImage);
    decodedImage.ease_isDecoded = YES;
    return decodedImage;
}
...
+ (UIImage *)decodedAndScaledDownImageWithImage:(UIImage *)image limitBytes:(NSUInteger)bytes {
    ...
    // on iOS7, do not forget to call [[EaseImageCache sharedImageCache] clearMemory];
    @autoreleasepool {
        ...
        EaseImageCopyAssociatedObject(image, destImage);
        destImage.ease_isDecoded = YES;
        return destImage;
    }
}
...
static inline CGAffineTransform EaseCGContextTransformFromOrientation(CGImagePropertyOrientation orientation, CGSize size) {
