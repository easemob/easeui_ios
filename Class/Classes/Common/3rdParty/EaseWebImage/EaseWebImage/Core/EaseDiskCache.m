...
#import "EaseFileAttributeHelper.h"
#import <CommonCrypto/CommonDigest.h>

static NSString * const EaseDiskCacheExtendedAttributeName = @"com.hackemist.EaseDiskCache";

@interface EaseDiskCache ()
...
#pragma mark - EasecachePathForKeyDiskCache Protocol
- (instancetype)initWithCachePath:(NSString *)cachePath config:(nonnull EaseImageCacheConfig *)config {
    if (self = [super init]) {
...
- (BOOL)containsDataForKey:(NSString *)key {
    ...
    // fallback because of https://github.com/rs/EaseWebImage/pull/976 that added the extension to the disk file name
...
- (NSData *)extendedDataForKey:(NSString *)key {
    ...
    NSData *extendedData = [EaseFileAttributeHelper extendedAttribute:EaseDiskCacheExtendedAttributeName atPath:cachePathForKey traverseLink:NO error:nil];
...
- (void)setExtendedData:(NSData *)extendedData forKey:(NSString *)key {
    ...
    if (!extendedData) {
        // Remove
        [EaseFileAttributeHelper removeExtendedAttribute:EaseDiskCacheExtendedAttributeName atPath:cachePathForKey traverseLink:NO error:nil];
    } else {
        // Override
        [EaseFileAttributeHelper setExtendedAttribute:EaseDiskCacheExtendedAttributeName value:extendedData atPath:cachePathForKey traverseLink:NO overwrite:YES error:nil];
    }
}
...
- (void)removeExpiredData {
    ...
    switch (self.config.diskCacheExpireType) {
        case EaseImageCacheConfigExpireTypeAccessDate:
            cacheContentDateKey = NSURLContentAccessDateKey;
            break;
        case EaseImageCacheConfigExpireTypeModificationDate:
            cacheContentDateKey = NSURLContentModificationDateKey;
            break;
        case EaseImageCacheConfigExpireTypeCreationDate:
            cacheContentDateKey = NSURLCreationDateKey;
            break;
        case EaseImageCacheConfigExpireTypeChangeDate:
...
- (nullable NSString *)cachePathForKey:(nullable NSString *)key inPath:(nonnull NSString *)path {
    NSString *filename = EaseDiskCacheFileNameForKey(key);
...
#define Ease_MAX_FILE_EXTENSION_LENGTH (NAME_MAX - CC_MD5_DIGEST_LENGTH * 2 - 1)

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
static inline NSString * _Nonnull EaseDiskCacheFileNameForKey(NSString * _Nullable key) {
    ...
    if (ext.length > Ease_MAX_FILE_EXTENSION_LENGTH) {
        ext = nil;
    }
