...
#import "NSBezierPath+EaseRoundedCorners.h"
#import <Accelerate/Accelerate.h>
#if Ease_UIKIT || Ease_MAC
#import <CoreImage/CoreImage.h>
#endif

static inline CGRect EaseCGRectFitWithScaleMode(CGRect rect, CGSize size, EaseImageScaleMode scaleMode) {
    rect = CGRectStandardize(rect);
    size.width = size.width < 0 ? -size.width : size.width;
    size.height = size.height < 0 ? -size.height : size.height;
    CGPoint center = CGPointMake(CGRectGetMidX(rect), CGRectGetMidY(rect));
    switch (scaleMode) {
        case EaseImageScaleModeAspectFit:
        case EaseImageScaleModeAspectFill: {
            if (rect.size.width < 0.01 || rect.size.height < 0.01 ||
                ...
                CGFloat scale;
                if (scaleMode == EaseImageScaleModeAspectFit) {
...
static inline UIColor * EaseGetColorFromPixel(Pixel_8888 pixel, CGBitmapInfo bitmapInfo) {
...
static inline CGImageRef _Nullable EaseCreateCGImageFromCIImage(CIImage * _Nonnull ciImage) {
...
@implementation UIImage (EaseTransform)

- (void)Ease_drawInRect:(CGRect)rect context:(CGContextRef)context scaleMode:(EaseImageScaleMode)scaleMode clipsToBounds:(BOOL)clips {
    CGRect drawRect = EaseCGRectFitWithScaleMode(rect, self.size, scaleMode);
...
- (nullable UIImage *)Ease_resizedImageWithSize:(CGSize)size scaleMode:(EaseImageScaleMode)scaleMode {
    ...
    EaseGraphicsImageRendererFormat *format = [[EaseGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    EaseGraphicsImageRenderer *renderer = [[EaseGraphicsImageRenderer alloc] initWithSize:size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
        [self Ease_drawInRect:CGRectMake(0, 0, size.width, size.height) context:context scaleMode:scaleMode clipsToBounds:NO];
    }];
    return image;
}
...
- (nullable UIImage *)Ease_roundedCornerImageWithRadius:(CGFloat)cornerRadius corners:(EaseRectCorner)corners borderWidth:(CGFloat)borderWidth borderColor:(nullable UIColor *)borderColor {
    EaseGraphicsImageRendererFormat *format = [[EaseGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    EaseGraphicsImageRenderer *renderer = [[EaseGraphicsImageRenderer alloc] initWithSize:self.size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
...
#else
            NSBezierPath *path = [NSBezierPath Ease_bezierPathWithRoundedRect:CGRectInset(rect, borderWidth, borderWidth) byRoundingCorners:corners cornerRadius:cornerRadius];
#endif
            [path closePath];
...
#else
            NSBezierPath *path = [NSBezierPath Ease_bezierPathWithRoundedRect:strokeRect byRoundingCorners:corners cornerRadius:strokeRadius];
#endif
            [path closePath];
...
- (nullable UIImage *)Ease_rotatedImageWithAngle:(CGFloat)angle fitSize:(BOOL)fitSize {
    ...
    EaseGraphicsImageRendererFormat *format = [[EaseGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    EaseGraphicsImageRenderer *renderer = [[EaseGraphicsImageRenderer alloc] initWithSize:newRect.size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
...
- (nullable UIColor *)Ease_colorAtPoint:(CGPoint)point {
...
#if Ease_UIKIT || Ease_MAC
    if (self.CIImage) {
        imageRef = EaseCreateCGImageFromCIImage(self.CIImage);
    }
#endif
    if (!imageRef) {
    ...
    CGImageRelease(imageRef);
    // Convert to color
    return EaseGetColorFromPixel(pixel, bitmapInfo);
}
...
- (nullable NSArray<UIColor *> *)Ease_colorsWithRect:(CGRect)rect {
    ...
    for (size_t index = start; index < end; index += 4) {
        ...
        UIColor *color = EaseGetColorFromPixel(pixel, bitmapInfo);
...
- (nullable UIImage *)Ease_blurredImageWithRadius:(CGFloat)blurRadius {
    ...
    if (CGImageGetBitsPerPixel(imageRef) != 32 ||
        ...
        EaseGraphicsBeginImageContextWithOptions(self.size, NO, self.scale);
        [self drawInRect:CGRectMake(0, 0, self.size.width, self.size.height)];
        imageRef = EaseGraphicsGetImageFromCurrentImageContext().CGImage;
    ...
    if (err != kvImageNoError) {
        NSLog(@"UIImage+EaseTransform error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", err, self);
        return nil;
    }
    err = vImageBuffer_Init(&scratch, effect.height, effect.width, format.bitsPerPixel, kvImageNoFlags);
    if (err != kvImageNoError) {
        NSLog(@"UIImage+EaseTransform error: vImageBuffer_Init returned error code %zi for inputImage: %@", err, self);
        return nil;
    }
