...
/**
 ...
 Call `setImage:` with `UIImage(NSImage)` which conforms to `EaseAnimatedImage` protocol will start animated image rendering. Call with normal UIImage(NSImage) will back to normal UIImageView(NSImageView) rendering
 ...
 */
@interface EaseAnimatedImageView : UIImageView
/**
 ...
 @warning Pay attention if you directly update the player's property like `totalFrameCount`, `totalLoopCount`, the same property on `EaseAnimatedImageView` may not get synced.
 */
@property (nonatomic, strong, readonly, nullable) EaseAnimatedImagePlayer *player;
...
/// Asynchronous setup animation playback mode. Default mode is EaseAnimatedImagePlaybackModeNormal.
@property (nonatomic, assign) EaseAnimatedImagePlaybackMode playbackMode;
...
/**
 ...
 @note The best practice to use incremental load is using `initWithAnimatedCoder:scale:` in `EaseAnimatedImage` with animated coder which conform to `EaseProgressiveImageCoder` as well. Then call incremental update and incremental decode method to produce the image.
...
/**
 If the image which conforms to `EaseAnimatedImage` protocol has more than one frame, set this value to `YES` will automatically
